...
This setup ensures that the role `bootstrap_ca_certs` is the producer of certificates into Vault's KV store, and the role `deploy_ca_certs` is the consumer of those certificates from the KV store.
...
bootstrap_ca_certs__vault_token: "{{ vault__openboa_token }}" # Securely manage this token!
  ...
  vars:
    bootstrap_ca_certs__ca_intermediate_certs_list:
      - commonName: "ca.dettonville.int"
        ...
        signerName: "{{ bootstrap_ca_certs__common_name }}" # Signed by the root CA
        country: US
        state: "New York"
        locality: "NYC"
