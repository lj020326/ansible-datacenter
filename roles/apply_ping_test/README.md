...
The role will place each target host into `host_offline` and `discovered_host_offline` groups if the ping test fails.  The `host_offline` is expected to be an inventory maintained group to explicitly defined hosts that have been bought offline for known reason (maintenance, upgrade, etc).  The `discovered_host_offline` is meant to support issue escalation for hosts that are expected to be online but fail the ping test and found to be offline.
