...
bootstrap_etcd__bin_dir_user: "{{ etcd_user }}"
...
bootstrap_etcd__download_url: "https://github.com/etcd-io/etcd/releases/download/v{{ etcd_version }}/etcd-v{{ etcd_version }}-linux-{{ bootstrap_etcd__architecture }}.tar.gz"
...
bootstrap_etcd__download_url_checksum: "sha256:https://github.com/coreos/etcd/releases/download/v{{ etcd_version }}/SHA256SUMS"
...
bootstrap_etcd__settings:
  ...
  "advertise-client-urls": "{{ 'https://' + hostvars[inventory_hostname]['ansible_' + bootstrap_etcd__interface].ipv4.address + ':' + bootstrap_etcd__client_port }}"
  "initial-advertise-peer-urls": "{{ 'https://' + hostvars[inventory_hostname]['ansible_' + bootstrap_etcd__interface].ipv4.address + ':' + bootstrap_etcd__peer_port }}"
  "listen-peer-urls": "{{ 'https://' + hostvars[inventory_hostname]['ansible_' + bootstrap_etcd__interface].ipv4.address + ':' + bootstrap_etcd__peer_port }}"
  "listen-client-urls": "{{ 'https://' + hostvars[inventory_hostname]['ansible_' + bootstrap_etcd__interface].ipv4.address + ':' + bootstrap_etcd__client_port + ',https://127.0.0.1:' + bootstrap_etcd__client_port }}"
...
The `etcd` default settings defined in `bootstrap_etcd__settings` can be overridden by defining a variable called `bootstrap_etcd__settings_user`. You can also add additional settings by using this variable. E.g. to override the default value for `log-output` setting and add a new setting like `grpc-keepalive-min-time` add the following settings to `group_vars/k8s.yml`:

```yaml
bootstrap_etcd__settings_user:
  "log-output": "stdout"
