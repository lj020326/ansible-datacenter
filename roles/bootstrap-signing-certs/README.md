...
include: ca-init.yml
when: bootstrap_signing_certs__ca_init is defined and ca_force_create == yes

include: certify_nodes.yml
when: bootstrap_signing_certs__ca_certify_nodes is defined and ca_force_certify_nodes

include: fetch_keys.yml
when: bootstrap_signing_certs__ca_fetch_certs is defined
...
- name: "Generate private key && Create root CA files"
  ...
  args:
    chdir: "{{ ca_certs_dir }}"
  with_items:
  - "openssl genrsa -out {{ bootstrap_signing_certs__caroot_key }} 2048"
  - "openssl req -config /usr/lib/ssl/openssl.cnf -new -key {{ bootstrap_signing_certs__caroot_key }} -x509 -days 1825 -out {{ bootstrap_signing_certs__caroot_cert }} -passin pass:{{ ca_rootca_password }} -subj \"{{ ca_subject }}\""
```
...
- name: "copy keys from ca_root_node to ansible machine for distribution"
  fetch: src="{{ ca_certs_dir }}/{{ item.ssl_key }}" dest="{{ bootstrap_signing_certs__cacert_keys_dir }}/{{ item.ssl_key }}" flat=yes
  with_items:
  - "{{ ca_node.ca_root_node }}"
  - "{{ ca_node.swarm.swarm-workers }}"
  - "{{ ca_node.swarm.swarm-managers }}"
...
- name: "copy certs from ca_root_node to ansible machine for distribution"
  fetch: src="{{ ca_certs_dir }}/{{ item.ssl_cert }}" dest="{{ bootstrap_signing_certs__cacert_certs_dir }}/{{ item.ssl_cert }}" flat=yes
  with_items:
  - "{{ ca_node.ca_root_node }}"
  - "{{ ca_node.swarm.swarm-workers }}"
  - "{{ ca_node.swarm.swarm-managers }}"
...
- name: "copy ca.pem ca-priv-key.pem"
  ...
  with_items:
  - "{{ bootstrap_signing_certs__caroot_cert }}"
  - "{{ bootstrap_signing_certs__caroot_key }}"
  - { src: "{{ ca_certs_dir }}/{{ bootstrap_signing_certs__caroot_cert }}", dest: "{{ bootstrap_signing_certs__cacert_certs_dir }}/{{ item }}" }
  - { src: "{{ ca_certs_dir }}/{{ bootstrap_signing_certs__pki_caroot_key }}", dest: "{{ bootstrap_signing_certs__cacert_keys_dir }}/{{ item }}" }
```

* Distribute the Certs & keys to the various nodes:

```yaml
- name: "Ensures {{ bootstrap_signing_certs__cacert_local_cert_dir }} and {{ bootstrap_signing_certs__cacert_local_key_dir }} dirs exist"
  file: path="{{ item }}" state=directory owner=root group=root mode=0750 recurse=yes
  with_items:
  - "{{ bootstrap_signing_certs__cacert_local_cert_dir }}"
  - "{{ bootstrap_signing_certs__cacert_local_key_dir }}"

- block:

   - name: "copy keys from ca_root_node to ansible machine for distribution"
     copy: src="{{ bootstrap_signing_certs__cacert_keys_dir }}/{{ item.ssl_key }}" dest="{{ bootstrap_signing_certs__cacert_local_key_dir }}/{{ item.ssl_key }}"
     with_items:
     - "{{ ca_node.ca_root_node }}"
     - "{{ ca_node.swarm.swarm-workers }}"
     - "{{ ca_node.swarm.swarm-managers }}"
   ...
   - name: "copy certs from ca_root_node to ansible machine for distribution"
     copy: src="{{ bootstrap_signing_certs__cacert_certs_dir }}/{{ item.ssl_cert }}" dest="{{ bootstrap_signing_certs__cacert_local_cert_dir }}/{{ item.ssl_cert }}"
     with_items:
     - "{{ ca_node.ca_root_node }}"
     - "{{ ca_node.swarm.swarm-workers }}"
     - "{{ ca_node.swarm.swarm-managers }}"
...
- name: "copy {{ bootstrap_signing_certs__pki_caroot_key }} to {{ bootstrap_signing_certs__cacert_local_key_dir }}"
  copy:
   src: "{{ bootstrap_signing_certs__cacert_keys_dir }}/{{ item }}"
   dest: "{{ bootstrap_signing_certs__cacert_local_key_dir }}/{{ item }}"
  with_items:
  - "{{ bootstrap_signing_certs__pki_caroot_key }}"

- name: "copy {{ bootstrap_signing_certs__caroot_cert }} to {{ bootstrap_signing_certs__ca_local_cert_dir }}"
  copy:
   src: "{{ bootstrap_signing_certs__cacert_certs_dir }}/{{ item }}"
   dest: "{{ bootstrap_signing_certs__ca_local_cert_dir }}/{{ item }}"
  with_items:
  - "{{ bootstrap_signing_certs__caroot_cert }}"
```
...
- hosts: caserver01
  ...
  vars:
   ...
   bootstrap_signing_certs__ca_force_certify_nodes: yes
  roles:
