...
The `deploy_pki_certs` Ansible role is a comprehensive solution for managing and deploying PKI certificates from a **Vault** (OpenBao or HashiCorp Vault) instance to target hosts. This role automates the entire certificate lifecycle, from initial deployment to validation and renewal, ensuring a secure and self-healing infrastructure.
...
1.  **Preparation & Dependencies:**
      ...
      * **Ensure Directories Exist:** Creates the necessary local directories on the target host to store certificates (`deploy_pki_certs__local_cert_dir`) and private keys (`deploy_pki_certs__local_key_dir`).
...
3.  **Service Route Certificates Deployment:**
      ...
      * **Loop Through Routes:** The role iterates over the `deploy_pki_certs__ca_service_routes_list` variable, processing each service route individually.
...
| :--- |:-------------------------------------------------------------------------------------------------------------| :--- |
| `deploy_pki_certs__vault_url` | **(Required)** The URL of the Vault server.                                                         | `https://vault.example.com:8200` |
| `deploy_pki_certs__vault_token` | **(Required)** The authentication token for Vault. **This should be stored securely in Ansible Vault.**      | None |
| `deploy_pki_certs__vault_mount_path` | **(Required)** The mount path of the PKI secrets engine used for issuing certificates (e.g., `pki-intermediate`). | `pki-intermediate` |
| `deploy_pki_certs__vault_pki_cert_role_name` | **(Required)** The name of the PKI role to use for issuing certificates (e.g., `ca_role`).                   | `ca_role` |
| `deploy_pki_certs__vault_kv_mount_path` | **(Required)** The mount path of the KV2 secrets engine where the CA trust chain is stored (e.g., `kv/pki`). | `kv/pki` |
| `deploy_pki_certs__certificate_ttl` | The validity period for issued certificates (e.g., `720h`).                                                  | `720h` |
| `deploy_pki_certs__ca_service_routes_list` | A list of service routes that require certificates on the host.                                              | `[]` |

-----

## Example Playbook Usage

To use this role, create a playbook like the following. Be sure to use Ansible Vault to manage your `deploy_pki_certs__vault_token`.
...
- name: Deploy and Manage Certificates with Vault
  ...
  vars:
    ...
    deploy_pki_certs__vault_pki_cert_role_name: "ca_role"
