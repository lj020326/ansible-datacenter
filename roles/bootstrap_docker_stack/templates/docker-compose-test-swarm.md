...
{% macro render_service_option(key, container, swarm_mode, filter_type='scalar', indent=4) %}
  {%- set swarm_restricted_keys = ['container_name', 'network_mode', 'restart', 'privileged', 'security_opt', 'links', 'devices', 'labels'] %}
  {%- set standalone_restricted_keys = ['secrets'] %}
  {%- set skip_key = (swarm_mode | bool and key in swarm_restricted_keys) or (not swarm_mode | bool and key in standalone_restricted_keys) %}
  {%- if not skip_key %}
    ...
    {{- render_service_option("labels", container, docker_stack__swarm_mode, filter_type='yaml') }}
        ...
        {%- if docker_stack__swarm_mode | d(False) | bool %}
            ...
            {%- if container.devices is defined %}
                {%- set _ = _deploy_config.update({ 'reservations': container.devices }) %}
            {%- endif %}
        {%- endif %}
