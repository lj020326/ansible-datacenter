```markdown
---
title: Ansible Role - bootstrap_jenkins
original_path: roles/bootstrap_jenkins/README.md
...
# Ansible Role: bootstrap_jenkins
...
- Newer versions of Jenkins require Java 8+ (see the test playbooks inside the `molecule/default` directory for an example of how to use newer versions of Java for your OS).
...
- **jenkins_plugins**:
  ...
  jenkins_plugins:
    ...
    - name: influxdb
      version: "1.12.1"
  ```
  - Jenkins plugins to be installed automatically during provisioning. Defaults to an empty list (`[]`). Items can use name or dictionary with `name` and `version` keys to pin specific versions of a plugin.
