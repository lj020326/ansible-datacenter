...
These dependencies are expected to be handled by prior plays using the `bootstrap_pip` and `bootstrap_docker` roles available in this repository.

### 2. Docker Runtime Dependency

The Docker runtime environment must be set up beforehand by a prior play using the `bootstrap_docker` role available in this repository.
...
This improved version maintains all original information while adhering to clean, professional Markdown formatting suitable for GitHub rendering. It includes a YAML frontmatter with additional metadata and a "Backlinks" section for context.