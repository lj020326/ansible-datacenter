```markdown
---
title: Jenkins Swarm Agent Setup Role
original_path: roles/bootstrap_jenkins_swarm_agent/README.md
category: Ansible Roles
tags: jenkins, swarm-agent, ansible, windows, ubuntu, centos
---

# Jenkins Swarm Agent Setup Role

This role sets up a Jenkins agent with the Swarm client on Windows, Ubuntu, or CentOS machines.
...
| `jenkins_swarm_agent__username`         | `sa_swarm_agent`                                                                                                                                       | User account to use while authenticating                                                          |
...
| `jenkins_swarm_agent__num_executors`    | `"{{ ansible_facts['processor_cores'] * 2 }}"`                                                                                                         | Number of executors for running jobs                                                              |
...
4. Use the "Add new Token" button to generate a new token and name it.
5. Copy the token when you generate it, as you cannot view it afterwards.
6. Revoke old tokens when no longer needed.

Reference:
- [How to get the API token for Jenkins](https://stackoverflow.com/questions/45466090/how-to-get-the-api-token-for-jenkins)
...
### Handling Jenkins with Context Path Prefix

If the Jenkins service is hosted with a context path prefix (e.g., `/jenkins`), specify the endpoint URL using the `jenkins_swarm_agent__controller_url` variable:
...
- [Jenkins Integration](/tags/jenkins)
```

This improved Markdown document is structured clearly, uses proper headings, and includes a "Backlinks" section for better navigation. The frontmatter has been enhanced with title, category, tags, and original path.